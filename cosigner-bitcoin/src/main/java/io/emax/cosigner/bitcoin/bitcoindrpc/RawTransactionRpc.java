...
public interface RawTransactionRpc {
   ...
   *        pay as a string (key) and the amount to pay that address (value) in bitcoins
   * 
   ...
   * @param verbose Set to 0 (the default) to return the serialized transaction as hex. Set to 1 to
   ...
   *        false (the default) to prevent Bitcoin Core from broadcasting the transaction if it
   *        includes a high fee. Transaction fees are the sum of the inputs minus the sum of the
   *        outputs, so this high fees check helps ensures user including a change address to return
   *        most of the difference back to themselves
   ...
   *        used to sign the transaction (even if the wallet has other matching keys). If this array
   *        is empty or not used, and wallet support is enabled, keys from the wallet will be used A
   *        private key in base58check format to use to create a signature for this transaction
   * @param sigHash The type of signature hash to use for all of the signatures performed. (You must
   *        use separate calls to the signrawtransaction RPC if you want to use different signature
   *        hash types for different signatures. The allowed values are: ALL, NONE, SINGLE,
   *        ALL|ANYONECANPAY, NONE|ANYONECANPAY, and SINGLE|ANYONECANPAY
   */

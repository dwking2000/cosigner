 ...
 * https://bitcoin.stackexchange.com/questions/1188/what-strategies-can-an-e-wallet-use-in-order-to-reduce-transaction-fees/1191#1191
...
public class BitcoinHelper {
    ...
    public static final int BITCOIN_AMOUNT_MAX_DIGITS = BITCOIN_AMOUNT_MAX_INTEGER + BITCOIN_AMOUNT_MAX_DECIMAL;
    ...
    public static int estimateTransactionSize(List<TxIn> inputs, List<TxOut> outputs) {
        ...
        return VERSION_SIZE + varIntSize(inputs.size()) + inputsSize + varIntSize(outputs.size()) + outputsSize + LOCKTIME_SIZE;
    }
    ...
    public static int estimateInputSize_ScriptSig(TxIn txIn) {
        ...
        case scripthash: {
            ...
            int redeemScriptSize = estimateRedeemScriptSize(txIn.getSignatures(), txIn.getOfSignatures());
            return OP_SIZE + txIn.getSignatures() * (varIntSize(signatureSize) + signatureSize) + OP_SIZE + varIntSize(redeemScriptSize)
                + redeemScriptSize;
        }
        case pubkeyhash: {
        ...
        default:
            throw new IllegalArgumentException("Unhandled ScriptPubKeyType=<" + txIn.getScriptPubKeyType() + ">");
        }
    }
    ...
    public static int estimateOutputSize_ScriptPubKey(TxOut txOut) {
        ...
        default:
            throw new IllegalArgumentException("Unhandled ScriptPubKeyType=<" + txOut.getScriptPubKeyType() + ">");
        }
    }
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        System.out.println("estimateOutputSize_ScriptPubKey(out)=" + estimateOutputSize_ScriptPubKey(out));
        ...
        System.out.println("2 input[scripthash,scripthash]/2 output[scripthash,scripthash] [ xxx bytes ] : "
            + estimateTransactionSize(Arrays.asList(inscript, inscript), Arrays.asList(outscript, outscript)));

        System.out.println("B) 1 input/2 output [ 258 bytes? ]: " + estimateTransactionSize(Arrays.asList(in), Arrays.asList(out_2)));
        System.out.println("C) 2 input/1 output [ 404 bytes? ]: " + estimateTransactionSize(Arrays.asList(in_2), Arrays.asList(out)));
        System.out.println("D) 12 input/2 output [ 1851 bytes? ]: " + estimateTransactionSize(Arrays.asList(in_12), Arrays.asList(out_2)));
        System.out.println("https://blockexplorer.com/tx/57488f3f8c3961dfa6e76b361aabb1f8e82fb08ea41746604ee6566dd52830de\n40 input/16 output [ 7.761 kilobytes ]: "

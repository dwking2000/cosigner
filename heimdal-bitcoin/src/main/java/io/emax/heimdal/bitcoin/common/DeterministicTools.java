...
public class DeterministicTools {
	 ...
	 * 
	 * @param userKeyPart
	 *            Expect these to be hex strings without the leading 0x
	 *            identifier
	 * @param serverKeyPart
	 *            Expect these to be hex strings without the leading 0x
	 *            identifier
	 ...
	 * @return
	 */
	public static String getDeterministicPrivateKey(String userKeyPart, String serverKeyPart, int rounds) {
		...
		System.arraycopy(privateKeyAttempt, 0, privateKey2, privateKey.length, privateKeyAttempt.length);
	...
	public static String getPublicAddress(String privateKey) {
		try {
			...
			byte[] networkPublicKeyBytes2 = new byte[networkPublicKeyBytes.length + publicRipemdKeyBytes.length];
			System.arraycopy(networkPublicKeyBytes, 0, networkPublicKeyBytes2, 0, networkPublicKeyBytes.length);
			System.arraycopy(publicRipemdKeyBytes, 0, networkPublicKeyBytes2, networkPublicKeyBytes.length,
	...
	public static byte[] getPublicKeyBytes(String privateKey) {
		try {
			...
			byte[] privateKeyBytes = Arrays.copyOfRange(networkPrivateKeyBytes, 1, networkPrivateKeyBytes.length);

...
public class TransactorTest extends TestCase {
  ...
  public void testCoinbase() throws Exception {
    ...
    Assert.assertTrue("Coinbase returns with a string",
        transactor.coinbase().getClass() == String.class);
    Assert.assertTrue("Coinbase " + transactor.coinbase() + " is hexadecimal",
        TestUtils.isHexNumber(transactor.coinbase()));

    System.out.println("Coinbase: " + transactor.coinbase());
    Assert.assertTrue("Coinbase " + transactor.coinbase()
    ...
    Assert.assertTrue("gasPriceAsHex " + transactor.gasPriceAsHex() + " is hexadecimal",
        TestUtils.isHexNumber(transactor.gasPriceAsHex()));
    System.out.println("balanceAsHex: " + monitor.balanceAsHex(transactor.coinbase()));
    Assert.assertTrue("balanceAsHex " + monitor.balanceAsHex(transactor.coinbase())
        + " is non-zero", !BigInteger.ZERO.equals(new BigInteger(monitor.balanceAsHex(
        transactor.coinbase()).substring(2), 16)));
  }
}

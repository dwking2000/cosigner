...
class MockTestTransactor implements Transactor {
  ...
  public Balance getBalance() throws Exception {
    return new Balance("0xDEADBEEFDEADBEEF", "5", currency, new BalanceConfirmation(
        Long.valueOf(1000), (long) 1000, "IMAFRIENDLYBLOCKCHAIN"), null, Boolean.TRUE);
  }

  public List<Balance> transfer(List<Transfer> transfers) {
    return Stream.concat(
        transfers.stream().map(
            transfer -> new Balance(transfer.from, BigInteger.valueOf(100)
                .subtract(new BigInteger(transfer.amount, 16)).toString(16), currency, null, null,
                Boolean.FALSE)),
        transfers.stream().map(
            transfer -> new Balance(transfer.to, BigInteger.valueOf(100)
                .add(new BigInteger(transfer.amount, 16)).toString(16), currency, null, null,
                Boolean.FALSE))).collect(Collectors.toList());
  }

  public Balance newAccount() {
    return new Balance("0xDEADBEEFDEADBEEF777", BigInteger.ZERO.toString(16), currency, null, null,
        Boolean.TRUE);
  }

  public Balance closeAccount(String account, String destinationAccountForRemainingBalance) {
    return new Balance(destinationAccountForRemainingBalance,
        BigInteger.valueOf(1000).toString(16), currency, null, null, Boolean.TRUE);
  }
}
  ...
  public void testTransfers() throws Exception {
    try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
      List<Transfer> transfers =
          Arrays.asList(new Transfer("to1", "from1", BigInteger.valueOf(7).toString(16), "TEST",
              null),

...
public class Monitor implements io.emax.heimdal.common.Ledger.Monitor {
   ...
   * @throws IOException
   */
  public String balanceAsHex(String address, Balance tag) throws IOException {

    List<String> params = asList(address, tag.toString().toLowerCase());
    return new Gson().fromJson(JSONRPC2.Call(uri, "eth_getBalance", params), Properties.class)
        .getProperty("result");
  }

  public String balanceAsHex(String address) throws IOException {
  ...
  public List<io.emax.heimdal.common.Balance> getBalances(List<String> accounts) throws Exception {
    ...
    // So... Get the hash for the latest and for the (latest - confirmation) blocks to build a
    // balance object for now

    // Get the latest block num
    List<String> params = Arrays.asList();
    String latestBlockNumber =
        new Gson().fromJson(JSONRPC2.Call(uri, "eth_blockNumber", params), Properties.class)
            .getProperty("result");

    // Get the latest block hash
    params = Arrays.asList(latestBlockNumber, "false");
    String latestBlockJson =
        new Gson().fromJson(JSONRPC2.Call(uri, "eth_getBlockByNumber", params), Properties.class)
            .getProperty("result");
    String latestBlockTimestamp =
        new Gson().fromJson(latestBlockJson, Properties.class).getProperty("timestamp");
    String latestBlockHash =
        new Gson().fromJson(latestBlockJson, Properties.class).getProperty("hash");

    // Get the confirmed block number and hash
    String confirmedBlockNumber =
        Long.toHexString(Long.parseLong(latestBlockNumber, 16) - confirmationsRequired);
    // And Hash
    params = Arrays.asList(confirmedBlockNumber, "false");
    String confirmedBlockJson =
        new Gson().fromJson(JSONRPC2.Call(uri, "eth_getBlockByNumber", params), Properties.class)
            .getProperty("result");
    String confirmedBlockTimestamp =
        new Gson().fromJson(confirmedBlockJson, Properties.class).getProperty("timestamp");
    String confirmedBlockHash =
        new Gson().fromJson(confirmedBlockJson, Properties.class).getProperty("hash");
    ...
    io.emax.heimdal.common.BalanceConfirmation confirmedBlock =
        new BalanceConfirmation(Long.parseLong(confirmedBlockTimestamp, 16), Long.parseLong(
            confirmedBlockNumber, 16), confirmedBlockHash);
    io.emax.heimdal.common.BalanceConfirmation latestBlock =
        new BalanceConfirmation(Long.parseLong(latestBlockTimestamp, 16), Long.parseLong(
            latestBlockNumber, 16), latestBlockHash);
    for (String account : accounts) {
      params = Arrays.asList(account, confirmedBlockNumber);
      String accountBalanceValue =
          new Gson().fromJson(JSONRPC2.Call(uri, "eth_getBalance", params), Properties.class)
              .getProperty("result");
      io.emax.heimdal.common.Balance accountBalance =
          new io.emax.heimdal.common.Balance(account, accountBalanceValue, this.getCurrency(),
              confirmedBlock, latestBlock, Boolean.FALSE);

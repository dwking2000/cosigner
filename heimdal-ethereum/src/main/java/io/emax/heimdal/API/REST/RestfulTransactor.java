  ...
  private Restlet getBalance(Context context) {
    return new Restlet(context) {
      ...
      public void handle(final Request request, final Response response) {
        ...
        try {
          response
              .setEntity(new Gson().toJson(transactor.getBalance()), MediaType.APPLICATION_JSON);
        ...
        try {
          if (entity.getMediaType() != null
            ...
            List<Transfer> transfers =
                new Gson().fromJson(input, new TypeToken<ArrayList<Transfer>>() {}.getType());
            response.setEntity(new Gson().toJson(transactor.transfer(transfers)),
  ...
  private Restlet getCurrency(Context context) {
    return new Restlet(context) {
      ...
      public void handle(final Request request, final Response response) {
        String input = "Default: Input was not read";
        try {
          response.setEntity(new Gson().toJson(transactor.getCurrency()),
              MediaType.APPLICATION_JSON);
          response.setStatus(Status.SUCCESS_OK);
        } catch (Exception e) {
          ServerError serverError = new ServerError(input, exceptionToString(e));
  ...
  private Restlet newAccount(Context context) {
    return new Restlet(context) {
      ...
      public void handle(final Request request, final Response response) {
        ...
        try {
          response
              .setEntity(new Gson().toJson(transactor.newAccount()), MediaType.APPLICATION_JSON);
          response.setStatus(Status.SUCCESS_OK);
        } catch (Exception e) {
          ServerError serverError = new ServerError(input, exceptionToString(e));
          response.setEntity(serverError.toString(), MediaType.TEXT_PLAIN);
      ...
      public void handle(final Request request, final Response response) {
        ...
        try {
          if (entity.getMediaType() != null
            ...
            String destinationAccountForRemainingBalance =
                args.get("destinationAccountForRemainingBalance").getAsString();
            String output =
                new Gson().toJson(transactor.closeAccount(account,

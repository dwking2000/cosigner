...
public class CurrencyConfiguration implements io.emax.heimdal.api.currency.CurrencyConfiguration {
	...
	public CurrencyConfiguration() {
		...
		try {
			...
			daemonConnectionString = heimdalProperties.getProperty("daemonConnectionString", daemonConnectionString);
			
			//minConfirmations
			try {
				int intParser = Integer
						.parseInt(heimdalProperties.getProperty("minConfirmations"));
			...
			//gasPrice
			try {
				long longParser = Long
						.parseLong(heimdalProperties.getProperty("gasPrice"));
				gasPrice = longParser;
			} catch (NumberFormatException nex) {
			}
			...
			try {
				long longParser = Long
						.parseLong(heimdalProperties.getProperty("simpleTxGas"));
			...
			try {
				long longParser = Long
						.parseLong(heimdalProperties.getProperty("contractGas"));
			...
			try {
				int intParser = Integer
						.parseInt(heimdalProperties.getProperty("minSignatures"));

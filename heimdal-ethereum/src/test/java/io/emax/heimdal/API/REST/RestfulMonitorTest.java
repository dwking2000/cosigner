...
class MockTestMonitor implements Monitor {
  ...
  public List<Balance> monitor(List<String> accounts) {
    return accounts
        .stream()
        .map(
            (s) -> new Balance(s, "5", currency, new BalanceConfirmation(Long.valueOf(1000),
                (long) 1000, "IMAFRIENDLYBLOCKCHAIN"), null, Boolean.TRUE))
        .collect(Collectors.toList());
  }

  public List<Balance> getBalances() {
    return Collections.singletonList(new Balance("0xDEADBEEF", "5", currency,
        new BalanceConfirmation(Long.valueOf(1000), (long) 1000, "IMAFRIENDLYBLOCKCHAIN"), null,
        Boolean.FALSE));
  }
...
public class RestfulMonitorTest {
  ...
  public void testPostMonitor() throws Exception {
    try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
      ...
      Balance[] expected =
          expectedMonitoredAccounts.toArray(new Balance[expectedMonitoredAccounts.size()]);
      String out = EntityUtils.toString(httpResponse.getEntity(), "UTF-8");
      Balance[] actual = new Gson().fromJson(out, Balance[].class);
      Assert.assertArrayEquals(expected, actual);
    }

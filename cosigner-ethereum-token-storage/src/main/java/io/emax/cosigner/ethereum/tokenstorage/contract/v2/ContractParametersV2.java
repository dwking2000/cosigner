package io.emax.cosigner.ethereum.tokenstorage.contract.v2;
...
public class ContractParametersV2 extends ContractParametersV1 {
  private static final Logger LOGGER = LoggerFactory.getLogger(ContractParametersV2.class);
  ...
  public Map<String, List<String>> parseTransfer(String bytecode) {
    ...
    parameters
        .put(ContractParametersInterface.NONCE, Collections.singletonList(intValue.toString(10)));
    ...
    parameters.put(ContractParametersInterface.SENDER, Collections.singletonList(readValue));

    // Recipient
    parameters
        .put(ContractParametersInterface.RECIPIENTS, parseArray(bufPointer, bytecode, false, true));
    bufPointer += paramSize;

    // Amount
    parameters
        .put(ContractParametersInterface.AMOUNT, parseArray(bufPointer, bytecode, true, true));
    bufPointer += paramSize;

    // SigV
    parameters.put(ContractParametersInterface.SIGV, parseArray(bufPointer, bytecode, false, true));
    bufPointer += paramSize;

    // SigR
    parameters.put(ContractParametersInterface.SIGR, parseArray(bufPointer, bytecode, false, true));
    bufPointer += paramSize;

    // SigS
    parameters.put(ContractParametersInterface.SIGS, parseArray(bufPointer, bytecode, false, true));
  ...
  public Map<String, List<String>> parseAdminFunction(String bytecode) {
    ...
    bufPointer += contract.getTransfer().length();
    HashMap<String, List<String>> parameters = new HashMap<>();
    parameters.put(ContractParametersInterface.FUNCTION, Collections.singletonList(function));
    LOGGER.debug("Found function: " + function);
    ...
    for (int i = 0; i < contract.getAdminParamTypes().get(function.toLowerCase()).size(); i++) {
      if (contract.getAdminParamTypes().get(function.toLowerCase()).get(i)
        ...
        parameters.put(ContractParametersInterface.PARAM + i, Collections.singletonList(readValue));
      } else {
        parameters.put(ContractParametersInterface.PARAM + i,
            parseArray(bufPointer, bytecode, false, false));
        bufPointer += paramSize;
      }
    }
  ...
  public String rebuildAdminFunction(Map<String, List<String>> params) {
    String function = params.get(ContractParametersInterface.FUNCTION).get(0);
    String result;
    switch (function) {
      case ContractV2.createTokens:
        result = createTokens(
            new BigInteger(params.get(ContractParametersInterface.NONCE).get(0)).longValue(),
            params.get(ContractParametersInterface.PARAM + 0).get(0), new BigInteger(1,
                ByteUtilities.toByteArray(params.get(ContractParametersInterface.PARAM + 1).get(0)))
                .longValue(), params.get(ContractParametersInterface.SIGV),
            params.get(ContractParametersInterface.SIGR),
            params.get(ContractParametersInterface.SIGS));
        break;
      case ContractV2.destroyTokens:
        result = destroyTokens(
            new BigInteger(params.get(ContractParametersInterface.NONCE).get(0)).longValue(),
            params.get(ContractParametersInterface.PARAM + 0).get(0), new BigInteger(1,
                ByteUtilities.toByteArray(params.get(ContractParametersInterface.PARAM + 1).get(0)))
                .longValue(), params.get(ContractParametersInterface.SIGV),
            ...
            params.get(ContractParametersInterface.SIGS));
        break;
      case ContractV2.setTokenContract:
        result = setTokenChild(
            new BigInteger(params.get(ContractParametersInterface.NONCE).get(0)).longValue(),
            params.get(ContractParametersInterface.PARAM + 0).get(0),
            params.get(ContractParametersInterface.SIGV),
            params.get(ContractParametersInterface.SIGR),
            params.get(ContractParametersInterface.SIGS));
        break;
      case ContractV2.reconcile:
        Map<String, BigInteger> balanceUpdates = new HashMap<>();
        for (int i = 0; i < params.get(ContractParametersInterface.PARAM + 0).size(); i++) {
          balanceUpdates.put(params.get(ContractParametersInterface.PARAM + 0).get(i),
              new BigInteger(ByteUtilities
                  .toByteArray(params.get(ContractParametersInterface.PARAM + 1).get(i))));
        }
        result = reconcile(
            new BigInteger(params.get(ContractParametersInterface.NONCE).get(0)).longValue(),
            balanceUpdates, params.get(ContractParametersInterface.SIGV),
            params.get(ContractParametersInterface.SIGR),
            params.get(ContractParametersInterface.SIGS));
        break;
      default:
  ...
  private List<String> parseArray(int bufPointer, String buffer, boolean decodeToInt,
    ...
    arrayPointer +=
        new ContractV2().getTransfer().length(); // Offset by function id, not included in calc.

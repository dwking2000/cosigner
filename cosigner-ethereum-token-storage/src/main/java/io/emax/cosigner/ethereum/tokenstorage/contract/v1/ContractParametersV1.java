package io.emax.cosigner.ethereum.tokenstorage.contract.v1;
...
public class ContractParametersV1 implements ContractParametersInterface {
    ...
    switch (function) {
      ...
      case ContractV1.reconcile:
        ...
        result = reconcile(new BigInteger(params.get(NONCE).get(0)).longValue(), balanceUpdates,
            params.get(SIGV), params.get(SIGR), params.get(SIGS));
        break;
      case ContractV1.scheduleVesting:
        result = scheduleVesting(new BigInteger(params.get(NONCE).get(0)).longValue(),
            ...
            params.get(SIGS));
        break;
      case ContractV1.calculateVesting:
        result =
  ...
  private List<String> parseArray(int bufPointer, String buffer, boolean decodeToInt) {
    ...
    arrayPointer +=
        new ContractV1().getTransfer().length(); // Offset by function id, not included in calc.
